...
# # Base PeptideAtlas URL (you may need to modify this if it's dynamic)
# base_url = 'https://db.systemsbiology.net/sbeams/cgi/PeptideAtlas/GetCoreProteomeMapping?&mapping_id=98&redundancy_constraint=on&apply_action=QUERY&atlas_build_id=572&peptideatlas_category_constraint=1'
...
# # massive_kb_peptides_df = pd.read_csv(massive_kb_peptides_file, sep='\t')

# # Since the massive_kb_peptides_file only has sequences, we can load it into a set
...
# # Function to fetch the PeptideAtlas page and find the "canonical" link

# def get_canonical_link(gene_accession):
#     base_url = "https://db.systemsbiology.net/sbeams/cgi/PeptideAtlas/GetProtein"
#     atlas_build_id = "572"  # Assuming this remains constant
#     url = f"{base_url}?atlas_build_id={atlas_build_id}&apply_action=QUERY&protein_name={gene_accession}"
#     return url

# ################################################################################################
# # Function to process the peptide table
...
#         fetch_peptide_observations(accession, sequence, gene_accession)
...
#     num_experiments = 0  # Count the number of experiments
